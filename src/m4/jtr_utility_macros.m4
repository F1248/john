...
if test -z "$OPENSSL_LIBS"; then
  if test -d /opt/homebrew/opt/openssl/lib; then
     ADD_LDFLAGS="$ADD_LDFLAGS -L/opt/homebrew/opt/openssl/lib"
  fi
fi
if test -z "$OPENSSL_CFLAGS"; then
  if test -d /opt/homebrew/opt/openssl/include; then
     ADD_CFLAGS="$ADD_CFLAGS -I/opt/homebrew/opt/openssl/include"
  fi
fi
